...
type PatientShellProps = {
  children: ReactNode;
  activeItem: "dashboard" | "requests" | "schedule" | "profile";
  pageSubtitle?: string;
};

type NavItem = {
  key: "dashboard" | "requests" | "schedule" | "profile";
  label: string;
  href: string;
...
const primaryNavItems: NavItem[] = [
  { key: "dashboard", label: "Dashboard", href: ROUTES.patient, icon: LayoutDashboard },
  { key: "requests", label: "Requests", href: ROUTES.patientRequest, icon: ClipboardList },
  { key: "schedule", label: "Schedule", href: ROUTES.patientSchedule, icon: CalendarDays },
  { key: "profile", label: "Profile", href: ROUTES.patientProfile, icon: User },
];

export function PatientShell({ children, activeItem, pageSubtitle }: PatientShellProps) {
        ...
        <section className="min-h-0 min-w-0 flex-1 overflow-hidden">
          <div className="h-full overflow-y-auto rounded-2xl bg-white shadow-xl shadow-[#8cb6cf]/15 backdrop-blur sm:rounded-3xl">
            <header className="sticky top-0 z-10 flex flex-wrap items-center gap-3 border-b border-[#d7e9f5] bg-white/70 px-3 py-3 backdrop-blur sm:px-6 sm:py-4">
              <div>
                <p className="font-display text-xl font-extrabold tracking-tight text-[#0d3f61]">Patient Workspace</p>
                ...
                <details className="relative">
                  ...
                  <div className="absolute right-0 top-12 z-20 w-56 rounded-2xl border border-[#d1e6f4] bg-white p-3 shadow-xl">
                    ...
                    <p className="mt-1 truncate text-sm font-medium text-zinc-900">
                      {userEmail ?? "patient@grandcure.com"}
                    </p>
                    <div className="mt-3">
                      <SignOutButton />
                    </div>
