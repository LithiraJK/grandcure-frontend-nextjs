...
function isPatientRole(role: string | undefined) {
	if (!role) {
		return false;
	}
...
export default function PatientProfilePage() {
	...
	useEffect(() => {
		if (!isAuthenticated || !isPatientRole(userRole)) {
	...
	const canRender = useMemo(() => isAuthenticated && isPatientRole(userRole), [isAuthenticated, userRole]);
	...
	const normalizedPhoneNumber = useMemo(() => normalizePhoneNumber(trimmedForm.phoneNumber), [trimmedForm.phoneNumber]);
	const isPhoneRegexValid = useMemo(() => /^\+?[0-9]{7,15}$/.test(normalizedPhoneNumber), [normalizedPhoneNumber]);
	...
	const isFormValid = useMemo(
		() =>
			Boolean(
				trimmedForm.phoneNumber && isPhoneValid && isPhoneRegexValid && isDateOfBirthValid && trimmedForm.address,
			),
	...
	const hasUnsavedChanges = useMemo(
		...
		[
			...
			trimmedInitialForm.address,
			trimmedInitialForm.dateOfBirth,
			trimmedInitialForm.phoneNumber,
		],
	);
	...
	if (isBootstrapLoading && !profile) {
		return (
			<PatientShell activeItem="profile" pageSubtitle="Patient Profile Update">
	...
	const handleProfileImageChange = (event: ChangeEvent<HTMLInputElement>) => {
		...
		setProfileImageFromFile(selected);
	};
	...
	const handleIdDrop = (event: DragEvent<HTMLLabelElement>) => {
		event.preventDefault();
		setActiveDropZone(null);
		const dropped = event.dataTransfer.files?.[0] ?? null;
		...
		const validationMessage = validateFile(dropped);
	...
	const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
		...
		try {
			...
			const message = error instanceof Error ? error.message : "Failed to update patient profile.";
			setToast({ message, tone: "error" });
		}
	};
	...
	const profileName = (toText((profile as Record<string, unknown> | null)?.name) || userEmail || "Patient").trim();
		...
		<PatientShell activeItem="profile" pageSubtitle="Patient Profile Update">
			<section className="mx-auto max-w-4xl rounded-3xl bg-[#f7f9fc] p-5 sm:p-8">
				<form className="space-y-6" onSubmit={handleSubmit}>
					{hasUnsavedChanges ? (
						<p className="rounded-xl bg-[#fff4df] px-4 py-3 text-sm font-medium text-[#7a541b]">You have unsaved changes.</p>
					) : null}
						...
						<div className="flex flex-col items-center text-center">
							...
							>
								...
								<label className="absolute -bottom-1 -right-1 inline-flex h-10 w-10 cursor-pointer items-center justify-center rounded-full bg-[#0d5f87] text-xl font-bold text-white shadow-md transition hover:bg-[#0b4f71]">
									...
									<input type="file" accept=".jpg,.jpeg,.png,.webp" className="hidden" onChange={handleProfileImageChange} />
								</label>
							</div>
					...
					<article className="rounded-2xl bg-white p-8 shadow-lg shadow-[#d8e3f0]/35">
						<h1 className="font-display text-3xl font-extrabold tracking-tight text-[#191c1e]">Personal Details</h1>
						<p className="mt-2 text-sm text-[#4e5960]">Keep your contact details up to date for better service coordination.</p>
					...
					<article className="rounded-2xl bg-white p-8 shadow-lg shadow-[#d8e3f0]/35">
						<h2 className="font-display text-3xl font-extrabold tracking-tight text-[#191c1e]">NIC Verification</h2>
						<p className="mt-2 text-sm text-[#4e5960]">Upload a clear NIC image (.jpg, .png, .webp, max 5MB).</p>
						...
						<div className="mt-6 grid gap-4">
							...
							>
								...
								<input type="file" accept=".jpg,.jpeg,.png,.webp" className="hidden" onChange={handleIdFileChange} />
							</label>
						</div>
			...
			{toast ? (
				<div
					className={[
						...
						toast.tone === "success" ? "bg-[#e8f6ef] text-[#1f6f48]" : "bg-[#fdeeee] text-[#9b2f2f]",
					].join(" ")}
					role="status"
					aria-live="polite"
				>
